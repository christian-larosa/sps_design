...
CREATE OR REPLACE TABLE `dh-darkstores-live.csm_automated_tables.ytd_sps_score_tableau_simple`
CLUSTER BY
   global_entity_id
AS
WITH
...
SELECT DISTINCT * FROM (
  ...
  UNION ALL
  SELECT global_entity_id, time_period, brand_sup, entity_key, division_type, supplier_level, time_granularity FROM `dh-darkstores-live.csm_automated_tables.ytd_sps_efficiency_simple`
  UNION ALL
...
LEFT JOIN `dh-darkstores-live.csm_automated_tables.ytd_sps_efficiency_simple` AS se
